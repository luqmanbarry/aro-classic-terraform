...
Each of the 3 OADP Helm charts is deployed and monitored by an Argo CD `Application` rendered from the cluster overlay configuration. Distinct application parameters are provided in the cluster values files. Use the `values.<cluster-name>.yaml` examples as reference.
...
modules:
  - name: oadp-operator
    config_path: gitops/apps/platform/oadp-operator
    sync_wave: 113
----
+
...
modules:
  - name: oadp-backup
    config_path: gitops/apps/platform/oadp-backup
    sync_wave: 113
----
+
...
modules:
  - name: oadp-restore
    config_path: gitops/apps/platform/oadp-restore
    sync_wave: 113
----
+
