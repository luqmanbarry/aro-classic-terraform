...
This module is intended to pair with dedicated ARO machine pools labeled for AI workloads, for example:

- `workload.platform/ai=true`

Default placement behavior is unconstrained:

- if you do not provide AI-specific scheduling manifests, OpenShift AI workloads use the default worker nodes
- if you want AI workloads on dedicated machine pools, add labels such as `workload.platform/ai=true` to those nodes and supply corresponding manifests through `hardwareProfiles`
